...
export const trackReducer = (tracks, { type, payload }) => {
    switch (type) {
        case 'SET_TRACK_VOLUME':
            return setTrackVolume(payload.value, payload.trackId, tracks);
        case 'SET_TRACK_SEND_LEVEL':
            return setTrackSendLevel(payload.value, payload.fxId, payload.trackId, tracks);
        case 'TRACK_MUTE_TOGGLE':
            return toggleTrackMute(payload.trackId, tracks);
        case 'TRACK_FX_TOGGLE':
            return toggleTrackFxBypass(payload.trackId, tracks);
        case 'TRACK_SET_READY_STATE':
            return setTrackReadyState(payload.state, payload.trackId, tracks);
        default:
            return tracks;
    }
};
