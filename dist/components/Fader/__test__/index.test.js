...
describe('<Fader />', () => {
    it('should renders without any errors', () => {
        const wrapper = shallow(React.createElement(Fader, null));
        expect(wrapper).toMatchSnapshot();
    });
    describe('value', () => {
        it('should be passed to thumb', () => {
            const value = 42.0;
            const wrapper = shallow(React.createElement(Fader, { value: value }));
    ...
    describe('isVertical', () => {
        it('should have isHorisontal class on false', () => {
            const wrapper = shallow(React.createElement(Fader, { isVertical: false }));
            const result = wrapper.hasClass('isHorisontal');
            expect(result).toBe(true);
        });
        it('should omit isHorisontal class on true', () => {
            const wrapper = shallow(React.createElement(Fader, { isVertical: true }));
            const result = wrapper.hasClass('isHorisontal');
    ...
    describe('onChange', () => {
        it('should be called on value change', () => {
            ...
            const wrapper = mount(React.createElement(Fader, { onChange: onChangeMock }));
